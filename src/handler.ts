...
export async function handle(context: Context, config: IConfig): Promise<void | Error> {
  ...
  if (!context.isBot) {
    ...
    if (type === "issue" && config.issues) {
      const currentLabels: string[] = (context.payload.issue.labels as [{ name: string }]).map((x) => {
        return x.name;
      });
    ...
    if (type === "pull_request" && config.pulls) {
      const currentLabels: string[] = (context.payload.pull_request.labels as [{ name: string }]).map((x) => {
        return x.name;
      });
...
async function addLabels(context: Context, labels: string[]) {
  ...
  .catch((err) => {
    throw new Error(`Couldn't add labels to issue: ${context.issue().number}, ${err}`);
  });
}
